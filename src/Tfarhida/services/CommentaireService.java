...
public class CommentaireService {
    ...
    public List<Commentaire> chargerComments(int idMaison){
        List<Commentaire> comments = new ArrayList<>();
        try {
            String sql = "select c.*, u.username from commentaire c "
             + "INNER JOIN user u ON (u.id = c.user_id) "
             + "WHERE c.maison_id = " + idMaison;
            ...
            while(rs.next()){
                ...
                m.setNom(rs.getString("username"));
                comments.add(m);
            }
