...
class CommentController extends AbstractController
{
    ...
    public function afficheCommentProduitJson(NormalizerInterface $normalizer, CommentRepository $repository){
        ...
        $jsonContent = $normalizer->normalize($comment, 'json',['groups'=>'produit']);
     ...
     */
    public function deleteCommentProduitJSON(NormalizerInterface $normalizer, $id)
    {
        $em = $this->getDoctrine()->getManager();
     ...
     * @return Response
     * @Route("/ajoutCommentProduitJSON/{id}/{id1}", name="ajoutCommentProduitJSON")
     */

    public function addCommentProduitJSON(\Symfony\Component\HttpFoundation\Request $request, SerializerInterface $serializer, $id,$id1){
        $em = $this->getDoctrine()->getManager();
        $produit =$em->getRepository(Produit::class)->find($id1);
        $user=$em->getRepository(User::class)->find($id);
        $content = $request->getContent();
        $data = $serializer->deserialize($content, comment::class, 'json');
        $produit->addComment($data);
        $user->addComment($data);

        $em->persist($produit,$user);
